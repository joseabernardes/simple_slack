...
import server.utils.ServerProtocol;

public class ReceiveFileServer extends Thread {
     ...
     * @param type
     */
    public ReceiveFileServer(int port, String fileName, int size, User receiver, User sender) {
        try {
            ...
            this.sender = sender;
            this.type = ReceiveFileServer.PRIVATE;
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
     ...
     * @param sender
     */
    public ReceiveFileServer(int port, String fileName, int size, Group group, User sender, DatagramSocket socketUDP) {
        try {
            socket = new ServerSocket(port);
            this.fileName = fileName;
            this.size = size;
            this.sender = sender;
            this.group = group;
            this.type = ReceiveFileServer.GROUP;
    ...
    private void saveFile(Socket clientSock) throws IOException {
        ...
        String path;
        if (type == ReceiveFileServer.PRIVATE) {
...
//        int filesize = 15123; // Send file size in separate msg
        ...
        if (type == ReceiveFileServer.PRIVATE) {
            ...
            outReceiver.println(makeJsonResponse(ServerProtocol.Private.FILE_SENDED, obj.toJSONString()));

        } else {
            byte[] buf = new byte[256];
            String res = makeJsonResponse("msg", obj.toJSONString()).toJSONString();
            buf = res.getBytes();
